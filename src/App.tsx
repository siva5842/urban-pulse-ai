...
import { 
  Camera, MapPin, ShieldCheck, CheckCircle2, 
  XCircle, Volume2, FileText, ChevronDown, ChevronUp, RefreshCw, 
  Clock, Radio, Truck, Car, Bike, Droplets, Zap, Navigation
} from 'lucide-react';
...
export default function App() {
  const [issues, setIssues] = useState<IssueRecord[]>(SEED_ISSUES);
  const [selectedIssue, setSelectedIssue] = useState<IssueRecord | null>(null);
  const [, setLoading] = useState(false);
  const [uploading, setUploading] = useState(false);
  const [currentGPS, setCurrentGPS] = useState<{ lat: number; lng: number }>({ lat: 12.0932, lng: 78.1841 });
  ...
  // 1. Fetch Issues with local fallback
      ...
      const popupHtml = `
        <div style="min-width: 250px; font-family: ui-sans-serif, system-ui, sans-serif; color: #0f172a;">
          ...
          <div style="display: flex; gap: 4px; margin-bottom: 6px;">
            <button onclick="window.urbanPulseVote && window.urbanPulseVote('${issue.id}', 'TRUE')" style="flex: 1; background-color: #10b981; color: white; border: none; border-radius: 4px; padding: 5px; font-size: 11px; font-weight: 600; cursor: pointer;">
              👍 Yes, True (${issue.true_votes})
            </button>
            <button onclick="window.urbanPulseVote && window.urbanPulseVote('${issue.id}', 'FALSE')" style="flex: 1; background-color: #ef4444; color: white; border: none; border-radius: 4px; padding: 5px; font-size: 11px; font-weight: 600; cursor: pointer;">
              👎 Fake (${issue.false_votes})
            </button>
          </div>

          <div style="display: flex; gap: 4px;">
            <button onclick="window.urbanPulseSpeak && window.urbanPulseSpeak('${issue.id}')" style="flex: 1; background-color: #3b82f6; color: white; border: none; border-radius: 4px; padding: 4px; font-size: 11px; font-weight: 600; cursor: pointer;">
              🔊 Read Aloud
            </button>
            <button onclick="window.urbanPulseInspect && window.urbanPulseInspect('${issue.id}')" style="flex: 1; background-color: #0f172a; color: white; border: none; border-radius: 4px; padding: 4px; font-size: 11px; font-weight: 600; cursor: pointer;">
              📋 View Work Order
            </button>
          </div>
  ...
  const handleVote = async (issueId: string, voteType: string) => {
    try {
      ...
      if (res.ok) {
        ...
        if (updated.is_escalated === 1 && updated.ticket_id) {
          showToastMsg(`🚨 Threshold Met! Work Order ${updated.ticket_id} Dispatched to ${updated.department}!`, "alert");
        }
        return;
      }
    } catch {
  ...
  const handleForceEscalate = async (issue: IssueRecord) => {
    setLoading(true);
    try {
    ...
    if (navigator.geolocation) {
      navigator.geolocation.getCurrentPosition(
        (pos) => {
          ...
          setIsOnSiteVerified(dist <= 50000);
        },
        () => {
  ...
  return (
    <div className="flex flex-col min-h-screen bg-slate-950 text-slate-100 font-sans selection:bg-cyan-500 selection:text-white">
      ...
      <main className="flex-1 grid grid-cols-1 lg:grid-cols-12 gap-0 overflow-hidden">
        ...
        {/* Right Side: Camera Capture & Feed */}
